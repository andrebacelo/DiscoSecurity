...
class CenaPerseguicao extends Phaser.Scene {
  ...
  init(data) {
    ...
    this.atributos = (data && data.atributos) || { chapeu: true, oculos: false };
  ...
  gerarNivel() {
    ...
    let proibidoMin = -1, proibidoMax = -1;
    let alvo;
    ...
    while (usados.length < alvo && tentativas < 100) {
      tentativas++;
      const x = R(400, 1700);
      if (x > proibidoMin && x < proibidoMax) continue;
      if (usados.some((u) => Math.abs(u - x) < 200)) continue;
      usados.push(x);
  ...
  tocarObstaculo(seguranca) {
    ...
    seguranca.body.setVelocityX(-420);
    seguranca.body.setVelocityY(-260);
